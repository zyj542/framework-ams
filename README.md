...
| [13-面试题库与自测清单.md](13-面试题库与自测清单.md) | 50 道 AMS 面试题：按主题分组、难度分级（无答案，先自测） | 练中学 |
| [14-面试题答案与解析-上篇.md](14-面试题答案与解析-上篇.md) | 题 1~21 答案解析（Binder/架构/启动/Activity/生命周期栈） | 练中学 |
| [15-面试题答案与解析-下篇.md](15-面试题答案与解析-下篇.md) | 题 22~50 答案解析（进程/Service/广播/Provider/客户端/实战/综合/附加） | 练中学 |
| [16-AOSP环境搭建与编译.md](16-AOSP环境搭建与编译.md) | 源码下载、编译、刷机、加日志验证 AMS 的全流程命令 | 环境必备 |
| [17-编译命令速查.md](17-编译命令速查.md) | make / m / mm / mma / mmm / banchan 区别与实战，增量编译提速 | 环境必备 |
| [examples/01-aidl-demo/](examples/01-aidl-demo/README.md) | 第 1 篇配套动手示例：ICompute AIDL 全流程 + Proxy/Stub 打包解包逐行分析 | 动手必备 |

**推荐阅读顺序**：01 → 02 → 03 → 04 → 05 → 10 → 06 → 07 → 08 → 09 → 11 → 12，
同时把 **16** 当作「环境搭建手册」随时查阅（编译、刷机、加日志都靠它）。
...
| 贯穿全程 | 环境搭建与编译 | 16 | 源码能下载、能编译、能刷机；能加日志验证 AMS 行为 |
| 贯穿全程 | 编译命令速查 | 17 | 会按「改哪编哪」选命令：全量用 m、单模块用 mma、加日志用 systemimage-nodeps |

> 16 篇不占正式周次：它更像「工具箱」，第 1 周就该把环境搭好（哪怕只编译通过一次），
> 之后每周的动手实验都会用到「加日志 → 编译 → 刷入 → logcat」这套闭环。
